...
export default function Home() {
  ...
  const [message, setMessage] = useState("STARTを押すと位置情報を取得します。");
    ...
    watchIdRef.current = navigator.geolocation.watchPosition(
      (pos) => {
        ...
        if (!previous) {
          ...
          setMessage("現在地を取得しました。少し移動すると速度を計算します。");
          return;
        }
        ...
        <section className="flex flex-1 flex-col px-7 pb-6 pt-10">
          ...
          <div className="mt-8 rounded-[2rem] bg-[#001B2A] p-6 text-white shadow-2xl">
            ...
            <button
              ...
              className={`mt-7 w-full rounded-2xl py-4 text-sm font-black tracking-[0.25em] transition hover:scale-[1.02] active:scale-[0.98] ${
                isRecording
                  ? "bg-[#7dd3fc] text-[#001B2A]"
                  : "bg-white text-[#001B2A]"
              }`}
            >
                ...
                {areaCells.map((cell) => {
                  ...
                  return (
                    <div key={cell.id} className={cellClass}>
                      {cell.isCurrentPosition && isRecording && (
